...
auto TransformMatrix::createPerspective2(float fieldOfView, float aspectRatio, float znear, float zfar) -> TransformMatrix
{
    auto f_n = 1.0f / (zfar - znear);
    auto theta = fieldOfView * 0.5f;
    SL_PANIC_IF(math::isZero(fmod(theta, math::piOver2), math::epsilon1))
...
auto TransformMatrix::createScale2(const glm::vec3 &scale) -> TransformMatrix
{
    Matrix result;
    result.m[0] = scale.x;
    result.m[5] = scale.y;
...
auto TransformMatrix::createRotationFromQuaternion2(const glm::quat &q) -> TransformMatrix
{
    auto x2 = q.x + q.x;
    auto y2 = q.y + q.y;
    auto z2 = q.z + q.z;
...
auto TransformMatrix::createRotationFromAxisAngle2(const glm::vec3 &axis, float angle) -> TransformMatrix
{
    auto x = axis.x;
    auto y = axis.y;
    auto z = axis.z;
...
auto TransformMatrix::createRotationX2(float angle) -> TransformMatrix
{
    auto c = cosf(angle);
    auto s = sinf(angle);
...
auto TransformMatrix::createRotationY2(float angle) -> TransformMatrix
{
    auto c = cosf(angle);
    auto s = sinf(angle);
...
auto TransformMatrix::createRotationZ2(float angle) -> TransformMatrix
{
    auto c = cosf(angle);
    auto s = sinf(angle);
...
auto TransformMatrix::createTranslation2(const glm::vec3 &translation) -> TransformMatrix
{
    Matrix result;
    result.m[12] = translation.x;
    result.m[13] = translation.y;
...
auto TransformMatrix::getScale2() const -> glm::vec3
{
    glm::vec3 result;
    decompose2(&result, nullptr, nullptr);
    return result;
}
...
auto TransformMatrix::getRotation2() const -> glm::quat
{
    glm::quat result;
    decompose2(nullptr, &result, nullptr);
    return result;
}
...
auto TransformMatrix::getTranslation2() const -> glm::vec3
{
    glm::vec3 result;
    decompose2(nullptr, nullptr, &result);
    return result;
}
...
auto TransformMatrix::transformPoint2(const glm::vec3 &point) const -> glm::vec3
{
    return {
        point.x * m[0] + point.y * m[4] + point.z * m[8] + m[12],
        point.x * m[1] + point.y * m[5] + point.z * m[9] + m[13],
...
auto TransformMatrix::transformDirection2(const glm::vec3 &dir) const -> glm::vec3
{
    return{
        dir.x * m[0] + dir.y * m[4] + dir.z * m[8],
        dir.x * m[1] + dir.y * m[5] + dir.z * m[9],
...
bool TransformMatrix::decompose2(glm::vec3 *scale, glm::quat *rotation, glm::vec3 *translation) const
{
    if (translation)
    {
        translation->x = m[12];
