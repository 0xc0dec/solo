...
{
    auto data = MeshData::fromFile(device, path, bufferLayout);
...
auto Mesh::fromFileAsync(Device *device, const str &path, const VertexBufferLayout &bufferLayout) -> sptr<AsyncHandle<Mesh>>
{
    ...
    auto producers = JobBase<Mesh>::Producers{[=]() { return fromFile(device, path, bufferLayout); }};
    auto consumer = [handle](const vec<sptr<Mesh>> &results) { handle->resolve(results[0]); };

    device->jobPool()->addJob(std::make_shared<JobBase<Mesh>>(producers, consumer));
