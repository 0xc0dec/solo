...
{
    rasterState = vk::createRasterizationStateInfo(false, false, VK_CULL_MODE_BACK_BIT, VK_FRONT_FACE_COUNTER_CLOCKWISE);
    multisampleState = vk::createMultisampleStateInfo(VK_SAMPLE_COUNT_1_BIT);
    blendAttachmentState = vk::createBlendAttachmentState(false,
        VK_BLEND_FACTOR_ZERO, VK_BLEND_FACTOR_ZERO, VK_BLEND_OP_ADD,
        VK_BLEND_FACTOR_ZERO, VK_BLEND_FACTOR_ZERO, VK_BLEND_OP_ADD,
        VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT);
    colorBlendState = vk::createColorBlendStateInfo(&blendAttachmentState, false, VK_LOGIC_OP_COPY);
}
...
void VulkanPipeline::setVertexShader(VkShaderModule shader, const char* entryPoint)
{
    vertexShader = shader;
    if (shader)
        vertexShaderStageInfo = vk::createShaderStageInfo(true, shader, entryPoint);
}
...
void VulkanPipeline::setFragmentShader(VkShaderModule shader, const char* entryPoint)
{
    ...
    if (shader)
        fragmentShaderStageInfo = vk::createShaderStageInfo(false, shader, entryPoint);
}
