...
static void validateNewAttachments(const vec<sptr<Texture2D>> &attachments)
{
    ...
    debugPanicIf(colorAttachmentsCount > GL_MAX_COLOR_ATTACHMENTS, "Too many color attachments");
}
...
auto OpenGLFrameBuffer::fromAttachments(const vec<sptr<Texture2D>> &attachments) -> sptr<OpenGLFrameBuffer>
{
    ...
    debugPanicIf(!result->handle_, "Unable to create frame buffer handle");
    ...
    debugPanicIf(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE, "Frame buffer has invalid state");
