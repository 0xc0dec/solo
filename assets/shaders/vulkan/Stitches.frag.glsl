...
void main()
{
    ...
    vec4 prevColor = texture(mainTex, vec2(uv0.x, uv0.y + 1.0 / variables.resolution.y), 1);
    ...
    if (stitchUvY > 0.5)
    {
        ...
        fragColor = texture(stitchTex, stitchUV, 1) * color;
        vec2 topStitchUV = vec2(nextStitchUvX, fract(stitchUvY - 0.5));
        vec4 topStitchColor = texture(stitchTex, topStitchUV, 1) * prevColor;
    ...
    else
    {
        ...
        fragColor = texture(stitchTex, stitchUV, 1) * prevColor;
        vec2 bottomStitchUV = vec2(stitchUvX, fract(stitchUvY + 0.5));
        vec4 bottomStitchColor = texture(stitchTex, bottomStitchUV, 1) * color;
